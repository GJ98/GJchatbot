...
class IntentProcessor(BaseProcessor):
    ...
    def __init__(self, model: nn.Module):
        """의도 분류 모델 Training, Inference 함수를 가진 클래스

        Args:
            model (nn.Module): 의도 분류 모델
        """
    ...
    def fit(self, 
        ...
        if label_dict != self.model.label_dict:
            raise Exception("(data lable dict) is not (model label dict)"
        ...
        for epoch in tqdm(range(self.epoch), desc='epoch', total=self.epoch):                
            ...
            for data in tqdm(tr_dataloader, desc='step', total=len(tr_dataloader)):
                ...
                loss = loss_fn(pred_label, label.long())
                loss.backward()
                optimizer.step()
    ...
    def predict(self, sequence: List[List[np.ndarray]]) -> str:
        """의도 분류 모델 Inference 함수

        Args:
            sequence (List[List[np.ndarray]]): 임베딩 문장
        ...
        pad_sequence = self.p.pad_question(sequence)
        pred_label = self.model(torch.tensor([pad_sequence]))[0]
        pred_class = pred_label.max(dim=-1)[1].tolist()
        return self.model.label_dict[pred_class]
    ...
    def __load_model(self):
        ...
        if self.model_loaded is False:
            ...
            state = torch.load(self.model_file + '.intent', 
    ...
    def __save_model(self):
        ...
        torch.save(state, self.model_file + '.intent')
